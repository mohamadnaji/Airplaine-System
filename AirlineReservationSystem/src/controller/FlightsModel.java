...
public class FlightsModel {
	...
	public static ArrayList<Flight> getAllFlights() throws SQLException {
		...
		while (rs.next()) {
			Flight flight = new Flight(rs.getInt(1), // flight id
					rs.getString(13), rs.getString(2), // airline_name
	...
	public static void addFlight(int id, String flight_number, String airline_name, int nbr_of_seats,
			int nbr_of_reserved_seats, String source, String destination, String arrival_time, String departure_time,
			LocalDate arrival_date, LocalDate departure_date) throws SQLException {
		int flag = 1;

		try {
			DB.InsertFun("INSERT INTO flight VALUES(" + id + ",'" + airline_name + "'," + flight_number + ","
					+ nbr_of_seats + "," + nbr_of_reserved_seats + ",'" + source + "','" + destination + "','"
...
//			DB.InsertFun("UPDATE flight SET flag = 0 WHERE flight_id = " + id + "");
	...
	public static void updateFlightByID(int id, String flight_number, String airline_name, int nbr_of_seats,
			int nbr_of_reserved_seats, String source, String destination, String arrival_time, String departure_time,
			LocalDate arrival_date, LocalDate departure_date) {

		try {
			DB.InsertFun("UPDATE flight SET airline_name = '" + airline_name + "',flight_number = '" + flight_number
					+ "',nbr_of_seats = " + nbr_of_seats + ",nbr_of_reserved_seats = " + nbr_of_reserved_seats
					+ ",source ='" + source + "',destination = '" + destination + "',arrival_time = '" + arrival_time
					+ "',departure_time ='" + departure_time + "',arrival_date = '" + arrival_date
					+ "',departure_date = '" + departure_date + "' WHERE flight_id = " + id + "");
		} catch (Exception e) {
			JOptionPane.showMessageDialog(null, e);
		}
